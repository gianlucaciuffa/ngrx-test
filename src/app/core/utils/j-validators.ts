...
export class JValidators {
  ...
  static minWord(minWord: number, optionsA: Opt = new Opt()): ValidatorFn {
    const options = new Opt();
    options.error = optionsA.error || true;
    options.level = optionsA.level || 'error';
    options.type = 'min';
    options.message = optionsA.message || 'Minimum number of words not reached {actual}/{minWord}';
    return (control: AbstractControl): ValidationErrors | null => {
      if (isEmptyInputValue(control.value) || isEmptyInputValue(minWord)) {
        return null;  // don't validate empty values to allow optional controls
      }
      const wordnum = control.value.split(' ').length;
      // Controls with NaN values after parsing should be treated as not having a
      // maximum, per the HTML forms spec: https://www.w3.org/TR/html5/forms.html#attr-input-max
      options.context = {minWord, actual: wordnum};
      return !isNaN(wordnum) && wordnum < minWord ? options : null;
    };
  }

  static maxWord(maxWord: number, optionsA: Opt = new Opt()): ValidatorFn {
    const options = new Opt();
    options.error = optionsA.error || true;
    options.level = optionsA.level || 'error';
    options.type = 'max';
    options.message = optionsA.message || 'Minimum number of words not reached {actual}/{minWord}';
    return (control: AbstractControl): ValidationErrors | null => {
      if (isEmptyInputValue(control.value) || isEmptyInputValue(maxWord)) {
        return null;  // don't validate empty values to allow optional controls
      }
      const wordnum = control.value.split(' ').length;
      // Controls with NaN values after parsing should be treated as not having a
      // maximum, per the HTML forms spec: https://www.w3.org/TR/html5/forms.html#attr-input-max
      options.context = {maxWord, actual: wordnum};
      return !isNaN(wordnum) && wordnum < maxWord ? options : null;
    };
  }
